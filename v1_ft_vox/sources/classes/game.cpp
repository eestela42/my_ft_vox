...
int game::init()
{
	...
	this->chunks = new ee::chunk[10];
	int x = -4;
	int y = -4;
	for (int i = 0; i < 10; i++)
	{
		std::cout << "chunk : " << i << std::endl;
		this->chunks[i] = ee::chunk(x, y);
		this->chunks[i].fill();
...
int game::initBuffers()
{
	...
	glVertexAttribPointer(1, 1, GL_INT, GL_FALSE, 3 * sizeof(float) + 2 * sizeof(int), (void*)(3 * sizeof(float)));
	glEnableVertexAttribArray(1);
	glVertexAttribPointer(2, 1, GL_INT, GL_FALSE, 3 * sizeof(float) + 2 * sizeof(int), (void*)(3 * sizeof(float) + sizeof(int)));
