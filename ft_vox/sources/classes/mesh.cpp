...
void mesh::createCube(int x, int y, int z)
{
	ThreeBitVariable 	count;
	unsigned int 		zero = this->base_vertexes.size();
	if (!zero)
		zero = 1;
	count.value = 0;
	...
	while(1)
	{
		...
		std::cout << bit;
		std::cout << std::endl;
		this->base_vertexes.push_back(pos);
		if (count.value == 7)
			break;
		count.value++;
	}
